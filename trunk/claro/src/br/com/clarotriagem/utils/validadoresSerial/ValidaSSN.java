...
public class ValidaSSN extends ValidadorGeral {

	public ValidaSSN(String serial) {
		super(serial);
	}

	public boolean valida() throws Exception {
		switch (new Character(serial.charAt(3))){
		...
		if (serial.length() != 11) {
			return false;
		} else if (!serial.substring(0, 1).equalsIgnoreCase("R") && !serial.substring(0, 1).equalsIgnoreCase("I") && !serial.substring(0, 1).equalsIgnoreCase("1")) {
			return false;
		} else if (Character.isDigit(serial.charAt(1))) {
			return false;
		} else if (!reAno.matcher(serial.substring(3, 4)).matches()) {
			return false;
		} else if (!reMes.matcher(serial.substring(4, 5)).matches()) {
			return false;
		} else {
			if (a > ano) {
				return false;
	...
	public static void main(String[] args){
		ValidaSSN vm = new ValidaSSN("846NNS28D8");
		try {
