			...
			.create(
					"tinymce.plugins.Safari",
					{
						init : function(f) {
							...
							g.editor = f;
							g.webKitFontSizes = [ "x-small", "small", "medium",
									"large", "x-large", "xx-large",
									"-webkit-xxx-large" ];
							g.namedFontSizes = [ "xx-small", "x-small",
									...
									.addCommand(
											...
											function(k, j) {
												...
												if (m
													...
													f.getDoc().execCommand(
															"CreateLink",
															false, j)
												}
											});
							f.onPaste.add( function(i, k) {
								function j(l) {
									l = l.target;
									if (l.nodeType == 1) {
										l.style.cssText = "";
										d(i.dom.select("*", l), function(m) {
											m.style.cssText = ""
										})
									}
								}
								a.add(i.getDoc(), "DOMNodeInserted", j);
								window.setTimeout( function() {
									a.remove(i.getDoc(), "DOMNodeInserted", j)
								}, 0)
							});
							f.onKeyUp
									.add( function(j, o) {
							...
							f.addCommand("FormatBlock", function(j, i) {
								var l = f.dom, k = l.getParent(f.selection
										.getNode(), l.isBlock);
								if (k) {
									...
									.addCommand(
											...
											function(j, i) {
												f.getDoc().execCommand(
														"InsertText", false,
														"mce_marker");
												f.getBody().innerHTML = f
														.getBody().innerHTML
														.replace(
																/mce_marker/g,
																f.dom
																		.processHTML(i) + '<span id="_mce_tmp">XX</span>');
												f.selection.select(f.dom
														.get("_mce_tmp"));
												f.getDoc().execCommand(
														"Delete", false, " ")
											});
													...
													d(
															...
															function(l) {
																...
																case "u":
																	...
																	break;
																case "sub":
																	j = "vertical-align: sub;";
																	break;
																case "sup":
																	j = "vertical-align: super;";
																	break;
																case "strike":
														...
														if (h
															...
															switch (h
																	.getAttrib(
																			...
																			"mce_name")) {
															case "font":
																if (!i.settings.convert_fonts_to_spans) {
																	h
																			.setAttrib(
																					m,
																					"style",
						...
						_fixAppleSpan : function(l) {
							...
							if (l.nodeName == "SPAN"
								...
								if (!j.convert_fonts_to_spans) {
									...
									if (h.color) {
										m.setAttrib(l, "mce_name", "font");
										m.setAttrib(l, "color", m
												.toHex(h.color))
									}
									if (h.backgroundColor) {
										m.setAttrib(l, "mce_name", "font");
										m.setStyle(l, "background-color",
												h.backgroundColor)
									}
								} else {
									if (h.fontSize) {
										m.setStyle(l, "fontSize", f[b(i,
												h.fontSize)])
									}
								}
						...
						_insertBR : function(f) {
							...
							if (h.getSel().focusNode == g.previousSibling) {
								h.select(j.insertAfter(j.doc
										.createTextNode("\u00a0"), g));
								h.collapse(1)
							}
							f.getWin().scrollTo(0,
									j.getPos(h.getRng().startContainer).y)
						}
					});
